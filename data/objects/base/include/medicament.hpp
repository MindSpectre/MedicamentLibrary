...
namespace drug_lib::data::objects
{
    ...
    public:
        ...
        Medicament(common::database::Uuid id, std::string name, std::string type, const bool requires_prescription,
        std::string approval_number, std::string approval_status,
                   std::string atc_code) :
            ObjectBase(std::move(id)), name_(std::move(name)), type_(std::move(type)),
            requires_prescription_(requires_prescription), approval_number_(std::move(approval_number)),
