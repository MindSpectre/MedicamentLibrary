...
namespace drug_lib::data::objects
{
    ...
    public:
        ...
        virtual Record to_record() const {
            Record newrec;
            newrec.add_field(std::make_shared<Field<std::string>>("name", name_));
            newrec.add_field(std::make_shared<Field<std::string>>("manufacturer", manufacturer_));
            newrec.add_field(std::make_shared<Field<double>>("price", price_));
            return std::move(newrec);
        }

        // Метод для обновления данных медикамента на основе записи
        virtual void from_record(const Record& record) {
            for (const auto& field : record.fields()) {
                if (field->name() == "name") {
                    name_ = dynamic_cast<Field<std::string>*>(field.get())->value();
                } else if (field->name() == "manufacturer") {
                    manufacturer_ = dynamic_cast<Field<std::string>*>(field.get())->value();
                } else if (field->name() == "price") {
                    price_ = dynamic_cast<Field<double>*>(field.get())->value();
                }
            }
        }
        ...
        virtual void from_json(const Json::Value& json) {
            ...
            if (json.isMember("manufacturer")) manufacturer_ = json["manufacturer"].asString();
