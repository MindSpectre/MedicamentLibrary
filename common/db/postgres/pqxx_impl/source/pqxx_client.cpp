...
namespace drug_lib::common::database
{
	...
	PqxxClient::PqxxClient(
		const std::string_view host, const uint32_t port, const std::string_view db_name,
		...
		: in_transaction_(false)
	{
		try
		{
			...
			conn_str << "host=" << host << " port=" << port << " user=" << login << " password=" << password <<
					" dbname=" << db_name;
	...
	void PqxxClient::create_database(
		const std::string_view host, const uint32_t port, const std::string_view db_name,
		const std::string_view login, const std::string_view password)
	// Build the connection string to connect to the 'template1' database
	{
		...
		conn_str << "host=" << host << " port=" << port << " user=" << login << " password=" << password <<
				" dbname=template1";
		// Connect to 'template1' to create a new database

		try
		{
			// Establish connection to the 'template1' database

			if (pqxx::connection trivial(conn_str.str());
				trivial.is_open())
			{
				...
				query << "CREATE DATABASE " << db_name << " WITH OWNER = " << login << " " << "ENCODING = 'UTF8' " <<
	...
	std::unique_ptr<pqxx::work> PqxxClient::initialize_transaction() const
	{
		...
		catch (std::exception &e)
		{
			throw TransactionException(
				std::string(e.what()).append("UNEXPECTED WORK THROWS ERROR"),
	...
	{
		if (const auto *deadlock_error = dynamic_cast<const pqxx::deadlock_detected *>(&pqxx_exception))
		{
			return DatabaseException(deadlock_error->what(), db_err::DEADLOCK_DETECTED);
		}

		if (const auto *rollback_error = dynamic_cast<const pqxx::transaction_rollback *>(&pqxx_exception))
		{
			return DatabaseException(rollback_error->what(), db_err::SYSTEM_ROLLBACK);
		}

		if (const auto *syntax_error = dynamic_cast<const pqxx::syntax_error *>(&pqxx_exception))
		{
			return DatabaseException(syntax_error->what(), db_err::INVALID_QUERY);
		}

		if (const auto *data_error = dynamic_cast<const pqxx::data_exception *>(&pqxx_exception))
		{
			return DatabaseException(data_error->what(), db_err::INVALID_DATA);
		}

		if (const auto *sql_error = dynamic_cast<const pqxx::sql_error *>(&pqxx_exception))
		{
		...
		return DatabaseException(pqxx_exception.what(), db_err::UNKNOWN_ERROR);
	}

	void PqxxClient::build_conflict_clause_for_force_insert(
		std::string &query, const std::string_view table_name,
	...
	void PqxxClient::build_returning_clause(
		std::string &query,
		const std::vector<std::shared_ptr<FieldBase>> &returning_fields)
	{
		std::ostringstream returning_stream;
		returning_stream << " RETURNING ";
		for (const auto &field: returning_fields)
		{
			returning_stream << field->get_name() << ", ";
		}
		query += returning_stream.str();
		query.resize(query.size() - 2);
	}

	void PqxxClient::conditions_to_query(
		const std::string_view table_name, std::ostringstream &query_stream,
		pqxx::params &params, uint32_t &param_index,
		const Conditions &conditions) const
	{
...

		{
			...
			{
				throw QueryException("No valid searchable fields defined for the table.", db_err::INVALID_DATA);
			}
	...
	void PqxxClient::oid_preprocess()
	{
		try
		{
			std::lock_guard lock(this->conn_mutex_);
			...
			const pqxx::result r = txn.exec(
				...
				"'float4', 'float8', 'text', 'varchar', 'bpchar', 'timestamp', 'timestamptz', 'uuid', 'json', 'jsonb')");
	...
	std::unique_ptr<FieldBase> PqxxClient::process_field(const pqxx::field &field) const
	{
		...
		// Handle float types
		if (type_oid->second == "jsonb" || type_oid->second == "json") // Double precision (64-bit float)
		{
			Json::Value json;
			Json::CharReaderBuilder reader_builder;
			std::string errs;
			if (std::istringstream json_stream(field.c_str());
				!parseFromStream(reader_builder, json_stream, &json, &errs))
			{
				throw std::runtime_error("Failed to parse JSON: " + errs);
			}
			field_ptr = std::make_unique<Field<Json::Value>>(field.name(), json);
			return field_ptr;
		}
	...
	void PqxxClient::make_unique_constraint(
	...
	void PqxxClient::create_table(const std::string_view table_name, const Record &field_list)
	{
		...
		{
			std::string name = escape_identifier(field->get_name());
			std::string type = field->get_sql_type_initialization();
	...
	void PqxxClient::remove_table(const std::string_view table_name)
	{
		const std::string table = escape_identifier(table_name);
		std::ostringstream query_stream;
		query_stream << "DROP TABLE IF EXISTS " << table;
	...
	bool PqxxClient::check_table(const std::string_view table_name)
	{
		...
		const std::string query = "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = " + this->
		                          conn_->quote(table) + ");";
	...
	void PqxxClient::upsert_implementation(
		const std::string_view table_name, const std::vector<Record> &rows,
	...
	void PqxxClient::upsert_implementation(
		const std::string_view table_name, std::vector<Record> &&rows,
		const std::vector<std::shared_ptr<FieldBase>> &replace_fields)
	{
		auto [query, params] = construct_insert_query(table_name, rows);
		build_conflict_clause_for_force_insert(query, table_name, replace_fields);
	...
	std::vector<Record> PqxxClient::insert_with_returning_implementation(
		const std::string_view table_name, const std::vector<Record> &rows,
		const std::vector<std::shared_ptr<FieldBase>> &returning_fields)
	{
		auto [query, params] = construct_insert_query(table_name, rows);
		build_returning_clause(query, returning_fields);
	...
	std::vector<Record> PqxxClient::upsert_with_returning_implementation(
		const std::string_view table_name, const std::vector<Record> &rows,
		const std::vector<std::shared_ptr<FieldBase>> &replace_fields,
		const std::vector<std::shared_ptr<FieldBase>> &returning_fields)
	{
		...
		std::vector<std::shared_ptr<FieldBase>> fields)
	{
		if (fields.empty())
		{
			throw QueryException("Invalid number of fts fields. Expected at least one element", db_err::INVALID_QUERY);
		} {
			std::lock_guard lock(this->conn_mutex_);
	...
	// Data Retrieval
	std::vector<Record> PqxxClient::select(const std::string_view table_name, const Conditions &conditions) const
	{
		if (conditions.empty())
		{
			throw QueryException(
				"Invalid number of conditions. For selecting all data call another function",
	...
	std::vector<std::unique_ptr<ViewRecord>> PqxxClient::view(
		const std::string_view table_name,
		const Conditions &conditions) const
	{
		if (conditions.empty())
	...
	void PqxxClient::remove(const std::string_view table_name, const Conditions &conditions)
	{
		...
		{
			throw QueryException(
				"Invalid number of conditions. For removing all data call another function",
				db_err::INVALID_QUERY);
		}
		const std::string table = escape_identifier(table_name);
		std::ostringstream query_stream;
	...
	uint32_t PqxxClient::count(const std::string_view table_name, const Conditions &conditions) const
	{
		if (conditions.empty())
		{
			throw QueryException(
				"Invalid number of conditions. For count all data call another function",
				db_err::INVALID_QUERY);
		}
		const std::string table = escape_identifier(table_name);
		std::ostringstream query_stream;
	...
	void PqxxClient::set_search_fields(
		const std::string_view table_name,
		std::vector<std::shared_ptr<FieldBase>> fields)
	{
		std::lock_guard lock(this->conn_mutex_);
	...
	void PqxxClient::set_conflict_fields(
		const std::string_view table_name,
		std::vector<std::shared_ptr<FieldBase>> fields)
	{
		std::lock_guard lock(this->conn_mutex_);
