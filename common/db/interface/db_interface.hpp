...
namespace drug_lib::common::database::interfaces
{
	...
	public:
		...
		template <RecordContainer Rows>
		std::vector<Record> insert_with_returning(std::string_view table_name, Rows &&rows,
		                                          const std::vector<std::shared_ptr<FieldBase>> &returning_fields)
		{
			return insert_with_returning_implementation(table_name, std::forward<Rows>(rows),
			                                            returning_fields);
		}

		template <RecordContainer Rows>
		std::vector<Record> upsert_with_returning(std::string_view table_name,
		                                          Rows &&rows,
		                                          const std::vector<std::shared_ptr<FieldBase>> &replace_fields,
		                                          const std::vector<std::shared_ptr<FieldBase>> &returning_fields)
		{
			return upsert_with_returning_implementation(table_name, std::forward<Rows>(rows),
			                                            replace_fields,
			                                            returning_fields);
		}
		...
		virtual void set_search_fields(std::string_view table_name,
		                               std::vector<std::shared_ptr<FieldBase>> fields) = 0;

		virtual void set_conflict_fields(std::string_view table_name,
		                                 std::vector<std::shared_ptr<FieldBase>> fields) = 0;
	...
	protected:
		...
		[[nodiscard]] virtual std::vector<Record>
		insert_with_returning_implementation(std::string_view table_name,
		                                     const std::vector<Record> &rows,
		                                     const std::vector<std::shared_ptr<FieldBase>> &returning_fields) = 0;
		...
		[[nodiscard]] virtual std::vector<Record>
		upsert_with_returning_implementation(std::string_view table_name,
		                                     const std::vector<Record> &rows,
		                                     const std::vector<std::shared_ptr<FieldBase>> &replace_fields,
		                                     const std::vector<std::shared_ptr<FieldBase>> &returning_fields) = 0;
