...
namespace drug_lib::data::objects
{
    ...
    {
        Patient patient;
        const auto current_diseases = PropertyFactory::create<patients::CurrentDiseases>(Json::Value(Json::arrayValue));
    ...
    {
        Patient patient;
        const auto current_diseases = PropertyFactory::create<patients::CurrentDiseases>(std::vector{1, 2, 3});
        patient.add_property(current_diseases);
    ...
    TEST(PatientPropertiesTest, GetPropertyInfo)
    {
        ...
        const auto current_diseases = PropertyFactory::create<patients::CurrentDiseases>(
