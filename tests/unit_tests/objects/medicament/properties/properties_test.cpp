...
TEST(MedicamentPropertiesTest, AddProperty)
{
    ...
    const auto symptoms = data::PropertyFactory::create<medicaments::PrescriptionDrug>(Json::Value());
...
TEST(MedicamentPropertiesTest, GetPropertyInfo)
{
    ...
    const auto prescription_ = data::PropertyFactory::create<medicaments::PrescriptionDrug>(std::move(prescription));
    ...
    for (const auto record = Medicament.to_record().fields(); const auto& field : record)
    {
        if (field->get_name() == Medicament::field_name::properties)
        {
            ...
            EXPECT_EQ(info[medicaments::properties::prescription]
                      [medicaments::PrescriptionDrug::names_of_json_fields::description]
                      .asString(),
