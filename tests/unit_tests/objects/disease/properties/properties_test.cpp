...
namespace drug_lib::data::objects
{
    ...
    {
        Disease disease;
        const auto symptoms = PropertyFactory::create<diseases::Symptoms>(Json::Value());
    ...
    TEST(DiseasePropertiesTest, GetPropertyInfo)
    {
        ...
        const auto symptoms = PropertyFactory::create<diseases::Symptoms>(std::move(symptoms_list));
        disease.add_property(symptoms);
