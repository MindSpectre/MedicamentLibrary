...
TEST(CurativeDrugsTest, ParameterizedConstructorWithVector)
{
	const std::vector diseases = {
		common::database::Uuid(),
		common::database::Uuid(),
		common::database::Uuid()
	};
	const CurativeDrugs curative_drugs(diseases);

	const Json::Value info = curative_drugs.get_info();
...
TEST(CurativeDrugsTest, FactoryCreate)
{
	...
	EXPECT_NO_THROW(data::PropertyFactory::create(properties::curative_drugs, Json::arrayValue));
}
