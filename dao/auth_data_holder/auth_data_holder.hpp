...
namespace drug_lib::dao
{
	...
	{
		constexpr char authentication_data[] = "authentication_data";
	}
	...
	public:
		...
		void remove_by_id(common::database::Uuid id) const
		{
			...
			removed_conditions.add_field_condition(
				common::database::make_field_unique<common::database::Uuid>(data::objects::auth_object::field_name::user_id),
				"=",
				common::database::make_field_unique<common::database::Uuid>("", std::move(id))
			);
		...
		[[nodiscard]] data::objects::AuthObject get_by_user_id(common::database::Uuid id) const
		{
			common::database::Conditions select_conditions;
			select_conditions.add_field_condition(
				common::database::make_field_unique<common::database::Uuid>(data::objects::auth_object::field_name::user_id),
				"=",
				make_field_unique<common::database::Uuid>("", std::move(id))
			);
	...
	private:
		...
		void setup() &
		{
			table_name_ = table_names::authentication_data;
			if (!connect_)
			{
				throw std::runtime_error("Cannot connect to database interface.");
			}
			//creating fields
			const auto ref_id_field = std::make_shared<common::database::Field<common::database::Uuid>>
					(data::objects::auth_object::field_name::user_id, common::database::Uuid().set_null());
			...
			const auto role_field = common::database::make_field_shared<std::string>(
				data::objects::auth_object::field_name::role);
			const auto salt_field = common::database::make_field_shared<std::string>(
				data::objects::auth_object::field_name::salt);
			const auto email_field = common::database::make_field_shared<std::string>(
				data::objects::auth_object::field_name::email);
			value_fields_.push_back(password_field);
			value_fields_.push_back(ref_id_field);
			value_fields_.push_back(salt_field);
			value_fields_.push_back(role_field);
			key_fields_.push_back(login_field);
			key_fields_.push_back(email_field);
